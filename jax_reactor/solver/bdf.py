...
def _get_common_params_and_coefficients(ode_fn,
                                        initial_time,
                                        initial_state,
                                        atol,
                                        rtol,
...
def _initialize_solver_internal_state(ode_fn,
                                      initial_time,
                                      initial_state,
                                      atol,
                                      rtol,
        ...
        p, e = _get_common_params_and_coefficients(ode_fn,
                                                   initial_time,
                                                   initial_state,
                                                   atol,
                                                   rtol,
        ...
        first_step_size = bdf_util.first_step_size(atol=atol,
        ...
        first_order_backward_difference = p.ode_fn_vec(initial_time, p.initial_state_vec) * first_step_size
        
        backward_differences = jnp.concatenate([p.initial_state_vec[jnp.newaxis,:],
                                           first_order_backward_difference[jnp.newaxis,:],
                                           jnp.zeros(jnp.array(jnp.stack((bdf_util.MAX_ORDER+1,p.num_odes)),dtype=jnp.int64))],
                                           axis=0)
...
def _solve(ode_fn,
    ...
    def maybe_step(_states):
        ...
        backward_differences = jnp.where(should_update_step_size,
        ...
        error_ratio = jnp.where(newton_converged,
                                bdf_util.error_ratio(next_backward_difference,
                                e.error_coefficients[order],
                                                        tol),
                                jnp.nan)
        ...
        # the step size.
        new_step_size = jnp.where(converged_and_rejected,
        ...
        backward_differences = jnp.where(accepted,
        ...
        num_steps_same_size = jnp.where(accepted, 
        ...
        for offset in [-1, +1]:
            ...
            new_order = jnp.where(should_update_order_and_step_size & proposed_error_ratio_is_lower,
                                    proposed_order, 
                                    new_order)
            new_error_ratio = jnp.where(should_update_order_and_step_size & proposed_error_ratio_is_lower,
                                        proposed_error_ratio,
                                        new_error_ratio)
        order = new_order
        error_ratio = new_error_ratio

        new_step_size = jnp.where(should_update_order_and_step_size,
                                bdf_util.next_step_size(step_size, order, error_ratio, p.safety_factor,
                                p.min_step_size_factor, p.max_step_size_factor),
                                new_step_size)
    ...
    solver_internal_state = _initialize_solver_internal_state(ode_fn,
                                                              initial_time,
                                                              initial_state,
                                                              atol,
                                                              rtol,
