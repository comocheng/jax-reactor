...
RECIPROCAL_SUMS = np.concatenate(
    [np.array([np.nan]), np.cumsum(1. / ORDERS[1:])])


def error_ratio(backward_difference: np.ndarray, error_coefficient: np.ndarray,
...
def first_step_size(atol: Union[np.ndarray, float],
  ...
  norm = np.linalg.norm(first_order_error_coefficient * second_derivative /
                        tol)
...
def interpolate_backward_differences(
  ...
  interpolated_backward_differences = np.concatenate([
      backward_differences[0].reshape(1,
                                      np.shape(backward_differences)[1]),
      interpolated_backward_differences_orders_one_to_five,
      np.zeros(
          np.stack(
              np.array([2, np.shape(backward_differences)[1]],
                       dtype=np.int64)))
  ],
                                                     axis=0)
...
@jax.jit
def newton_qr(
    jacobian_mat: np.ndarray, newton_coefficient: Union[np.ndarray, float,
                                                        int],
...
def newton(
    backward_differences: np.ndarray, max_num_iters: Union[np.ndarray, float,
                                                           int],
    newton_coefficient: Union[np.ndarray, float, int], ode_fn_vec: Callable,
    order: Union[np.ndarray, float, int], step_size: Union[np.ndarray, float,
                                                           int],
    time: Union[np.ndarray, float,
                int], tol: Union[np.ndarray,
                                 float], unitary: np.ndarray, upper: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
  """Runs Newton's method to solve the BDF equation."""
  initial_guess = np.sum(np.where(
      np.arange(MAX_ORDER + 1).reshape(-1, 1) <= order,
      backward_differences[:MAX_ORDER + 1],
      np.zeros_like(backward_differences)[:MAX_ORDER + 1]),
                         axis=0)

  rhs_constant_term = newton_coefficient * np.sum(np.where(
  ...
  def newton_body(iterand):
    ...
    delta = np.squeeze(
        jax.scipy.linalg.solve_triangular(upper,
                                          np.matmul(np.transpose(unitary),
                                                    rhs[:, np.newaxis]),
                                          lower=False))
...
def next_step_size(step_size, order, error_ratio, safety_factor,
  ...
  return step_size * np.clip(safety_factor * factor, min_step_size_factor,
