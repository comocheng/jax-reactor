...
def test_first_step_size_is_large_when_ode_fn_is_constant():
    initial_state_vec = np.array([1.], dtype=np.float64)
    ...
    initial_time = np.array(0., dtype=np.float64)
    ...
    onp.testing.assert_allclose(onp.asarray(max_step_size,dtype=onp.float64),onp.asarray(step_size,dtype=onp.float64),err_msg='step size is not equal')

def test_interpolation_matrix_unit_step_size_ratio():
    order = np.array(bdf_util.MAX_ORDER, dtype=np.int32)
    step_size_ratio = np.array(1., dtype=np.float64)
    ...
    onp.testing.assert_allclose(
        interpolation_matrix, np.array([
            ...
            [-4., 6., -4., 1., 0.],
            [-5., 10., -10., 5., -1.]
        ],dtype=np.float64),err_msg='Interpolation matrices are not equal')
...
def test_interpolate_backward_differences_zeroth_order_is_unchanged():
    ...
    onp.testing.assert_allclose(
        ...
        interpolated_backward_differences[0],err_msg='Interpolated backward differences are not equal')

def test_newton_order_one():
    jacobian_mat = np.array([[-1.]], dtype=np.float64)
    ...
    backward_differences = np.array([[1.], [-1.], [0.], [0.], [0.], [0.]],
                                       dtype=np.float64)
    ...
    exact_next_state = ((1. - bdf_coefficient) * state + bdf_coefficient) / (
        1. + step_size - bdf_coefficient)

    onp.testing.assert_allclose(
        next_backward_difference,
        exact_next_state)
    onp.testing.assert_allclose(
        next_state, exact_next_state)
