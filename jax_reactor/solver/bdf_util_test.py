...
def test_first_step_size_is_large_when_ode_fn_is_constant():
  ...
  onp.testing.assert_allclose(onp.asarray(max_step_size, dtype=onp.float64),
...
def test_interpolate_backward_differences_zeroth_order_is_unchanged():
  backward_differences = np.array(onp.random.normal(size=((bdf_util.MAX_ORDER +
                                                           3, 3))),
                                  dtype=np.float64)
