...
# local imports
from .dataclass import GasInfo, KineticsCoeffs, KineticsData
...
def calculate_rate_constants(T: float, R: float, params: np.ndarray) -> np.ndarray:
    A, b, Ea = params
    k = A * (T ** b) * (np.exp(-Ea * 4184.0 / R / T))
    return k


vmap_arrhenius = jax.jit(
    jax.vmap(calculate_rate_constants, (None, None, 0))
)  # vmap and jit once
