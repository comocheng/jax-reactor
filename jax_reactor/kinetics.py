...
def get_equilibirum_constants(T:float,
                              P:float, 
                              R:float, 
                              sdivR:np.ndarray,
                              hdivRT:np.ndarray,
...
def calculate_rate_constants(T:float,
                             R:float,
...
def get_forward_rate_constants(T:float, 
                               R:float, 
                               C:np.ndarray,
    ...
    three_body_k = np.multiply(initial_k[kinetics_data.three_body_indices], 
    ...
    total_falloff_indices = np.concatenate([kinetics_data.falloff_indices,
    ...
    Pr = np.divide(np.multiply(k0[total_falloff_indices], 
                    C_M[total_falloff_indices]), kinf) # calculate Pr
    log10Pr = np.log10(Pr)
    falloff_k = np.multiply(k[total_falloff_indices],(Pr/(1.+Pr))) # update all type of falloff
    kf = jax.ops.index_update(k, 
                              jax.ops.index[total_falloff_indices],
                              falloff_k)
    F = troe_falloff_correction(T,
    ...
    final_k = jax.ops.index_update(kf, 
                                   jax.ops.index[kinetics_data.troe_falloff_indices], 
                                   troe_k)    # final update
    return final_k                                             


def get_reverse_rate_constants(kf:np.ndarray, 
                               Kc:np.ndarray, 
