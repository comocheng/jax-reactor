...
class NASAPolynomials(
  ...
  def __new__(cls, low_T_nasa_poly, high_T_nasa_poly, temp_mid_points):
    return super(NASAPolynomials,
                 cls).__new__(cls, low_T_nasa_poly, high_T_nasa_poly,
                              temp_mid_points)
...
class KineticsCoeffs(
    namedtuple("KineticsCoeffs", [
        "arrhenius_coeffs", "arrhenius0_coeffs", "troe_coeffs",
        "efficiency_coeffs"
    ])):
  """
    namedtuple class to store arrhenius parameters for all reaction types 
    """
  def __new__(cls, arrhenius_coeffs, arrhenius0_coeffs, troe_coeffs,
              efficiency_coeffs):
    return super(KineticsCoeffs,
                 cls).__new__(cls, arrhenius_coeffs, arrhenius0_coeffs,
...
class KineticsData(
    namedtuple("KineticsData", [
        "three_body_indices", "troe_falloff_indices", "falloff_indices",
        "is_reversible"
    ])):
  """
    namedtuple class to store indicies for reaction types and reversible reactions
    """
  def __new__(cls, three_body_indices, troe_falloff_indices, falloff_indices,
              is_reversible):
    return super(KineticsData,
                 cls).__new__(cls, three_body_indices, troe_falloff_indices,
...
class GasInfo(
  ...
  """
    ...
    """
  def __new__(cls, reactant_stioch_coeffs, product_stioch_coeffs,
              molecular_weights):
    return super(GasInfo,
                 cls).__new__(cls, reactant_stioch_coeffs,
                              product_stioch_coeffs, molecular_weights)
...
class ProductionRates(
    namedtuple("ProductionRates", [
        "forward_rates_of_progress", "reverse_rates_of_progress", "qdot",
        "wdot"
    ])):
  """
    namedtuple class to store 
    """
  def __new__(cls, forward_rates_of_progress, reverse_rates_of_progress, qdot,
              wdot):
    return super(ProductionRates,
                 cls).__new__(cls, forward_rates_of_progress,
                              reverse_rates_of_progress, qdot, wdot)
...
class Parameters(
    namedtuple("Parameters", [
        ...
        "equilibrium_constants", "enthalpy_mole", "entropy_mole",
        "heat_capacity_mole"
    ])):
  def __new__(cls, forward_rate_constants, reverse_rate_constants,
              equilibrium_constants, enthalpy_mole, entropy_mole,
              heat_capacity_mole):
    return super(Parameters,
                 cls).__new__(cls, forward_rate_constants,
                              reverse_rate_constants, equilibrium_constants,
                              enthalpy_mole, entropy_mole, heat_capacity_mole)
...
class EnthalpyData(
    namedtuple("EnthalpyData",
               ["hdivRT", "partial_molar_enthalpies", "h_mole"])):
  """
    namedtuple class to store heat capacity data 
    """
  def __new__(cls, hdivRT, partial_molar_enthalpies, h_mole):
    return super(EnthalpyData, cls).__new__(cls, hdivRT,
                                            partial_molar_enthalpies, h_mole)
...
class EntropyData(
  ...
  def __new__(cls, sdivR, partial_molar_entropies, s_mole):
    return super(EntropyData, cls).__new__(cls, sdivR, partial_molar_entropies,
                                           s_mole)
...
def get_nasa_polynomials_from_yaml(loaded_yaml: dict) -> NASAPolynomials:
  ...
  temp_mid_points = onp.ones(
      (n_species), dtype=onp.int64) * 1000.  #assign typical T
  for i in range(n_species):
    low_T_nasa_poly[i, :] = loaded_yaml['species'][i]['thermo']['data'][0]
    high_T_nasa_poly[i, :] = loaded_yaml['species'][i]['thermo']['data'][1]
    temp_mid_points[i] = loaded_yaml['species'][i]['thermo'][
        'temperature-ranges'][1]
  return NASAPolynomials(low_T_nasa_poly=np.array(low_T_nasa_poly,
                                                  dtype=np.float64),
                         high_T_nasa_poly=np.array(high_T_nasa_poly,
                                                   dtype=np.float64),
                         temp_mid_points=np.array(temp_mid_points,
                                                  dtype=np.int64))
...
def get_kinetic_parameters_from_yaml(
  ...
  for i in range(gas.n_reactions):
    ...
    if 'units' in loaded_yaml:
      if loaded_yaml['units']['length'] == 'cm' and loaded_yaml['units'][
          'quantity'] == 'mol':
        ...
        if gas.reaction_type(i) in [4]:
          ...
          reactions[i]['A_0'] *= (1e-3)**(reactant_stoich_coeffs[:, i].sum() -
                                          1)
