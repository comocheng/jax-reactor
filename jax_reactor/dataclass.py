...
class NASAPolynomials(namedtuple("NASAPolynomials",
                             ["low_T_nasa_poly",
                              "high_T_nasa_poly",
                              "temp_mid_points",])):
    """
...
class KineticsCoeffs(namedtuple("KineticsCoeffs",["arrhenius_coeffs",
                                         "arrhenius0_coeffs",
                                         "troe_coeffs",
                                         "efficiency_coeffs"])):
    """
    ...
    """
    def __new__(cls, arrhenius_coeffs, arrhenius0_coeffs, troe_coeffs, efficiency_coeffs):
        return super(KineticsCoeffs,cls).__new__(cls, arrhenius_coeffs, arrhenius0_coeffs, 
                                                 troe_coeffs, efficiency_coeffs)

register_pytree_namedtuple(KineticsCoeffs) #JAX pytree 



class KineticsData(namedtuple("KineticsData",["three_body_indices",
    ...
    """                                      
    def __new__(cls, three_body_indices, troe_falloff_indices, falloff_indices, is_reversible):
        return super(KineticsData,cls).__new__(cls, three_body_indices, troe_falloff_indices, 
                                                 falloff_indices, is_reversible)
register_pytree_namedtuple(KineticsData) #JAX pytree 


class GasInfo(namedtuple("GasInfo",["reactant_stioch_coeffs",
                                    "product_stioch_coeffs",
                                    "molecular_weights"])):
    """
    namedtuple class to store 
    """                                      
    def __new__(cls, reactant_stioch_coeffs, product_stioch_coeffs, molecular_weights):
        return super(GasInfo,cls).__new__(cls, reactant_stioch_coeffs, product_stioch_coeffs, 
                                                 molecular_weights)
register_pytree_namedtuple(GasInfo) #JAX pytree 


class ProductionRates(namedtuple("ProductionRates",["forward_rates_of_progress",
    ...
    def __new__(cls, forward_rates_of_progress, reverse_rates_of_progress, qdot, wdot):
        return super(ProductionRates,cls).__new__(cls, forward_rates_of_progress, reverse_rates_of_progress, 
                                                 qdot, wdot)
register_pytree_namedtuple(ProductionRates) #JAX pytree 

class Parameters(namedtuple("Parameters",["forward_rate_constants",
                                         "reverse_rate_constants",
                                         "equilibrium_constants",
                                         "enthalpy_mole",
                                         "entropy_mole",
                                         "heat_capacity_mole"])):
    def __new__(cls, forward_rate_constants, 
                     reverse_rate_constants,
                     equilibrium_constants,
                     enthalpy_mole,
                     entropy_mole,
                     heat_capacity_mole):
        return super(Parameters, cls).__new__(cls,forward_rate_constants, 
                                                  reverse_rate_constants,
                                                  equilibrium_constants,
                                                  enthalpy_mole,
                                                  entropy_mole,
...
class CpData(namedtuple("CpData",["cpdivR","cp_mole"])):
    ...
    def __new__(cls, cpdivR, cp_mole):
        return super(CpData, cls).__new__(
            cls, cpdivR, cp_mole)

register_pytree_namedtuple(CpData) #JAX pytree 


class EnthalpyData(namedtuple("EnthalpyData",["hdivRT","partial_molar_enthalpies","h_mole"])):
    """
    namedtuple class to store heat capacity data 
    """
...
class EntropyData(namedtuple("EntropyData",["sdivR","partial_molar_entropies","s_mole"])):
    """
    namedtuple class to store heat capacity data 
    """
...
def get_nasa_polynomials_from_yaml(loaded_yaml: dict)->NASAPolynomials:
    ...
    temp_mid_points = onp.ones((n_species),dtype=onp.int64) * 1000. #assign typical T
    for i in range(n_species):
        low_T_nasa_poly[i,:]= loaded_yaml['species'][i]['thermo']['data'][0]
        high_T_nasa_poly[i,:] = loaded_yaml['species'][i]['thermo']['data'][1]
        temp_mid_points[i] = loaded_yaml['species'][i]['thermo']['temperature-ranges'][1]
    return NASAPolynomials(low_T_nasa_poly=np.array(low_T_nasa_poly,dtype=np.float64),
                          high_T_nasa_poly=np.array(high_T_nasa_poly,dtype=np.float64),
                          temp_mid_points=np.array(temp_mid_points,dtype=np.int64))


def get_kinetic_parameters_from_yaml(loaded_yaml:dict, 
    ...
    for i in range(gas.n_reactions):
        ...
        if gas.reaction_type(i) in [1, 2]:
            ...
            if type(loaded_yaml['reactions'][i]['rate-constant']['Ea']) is str:
                Ea = onp.float64([loaded_yaml['reactions'][i]['rate-constant']['Ea'].split(' ')[0]])
            else:
        ...
        if gas.reaction_type(i) in [2]:
            ...
            if 'efficiencies' in loaded_yaml['reactions'][i]:
                reactions[i]['efficiencies'] = loaded_yaml['reactions'][i]['efficiencies']
        ...
        if 'units' in loaded_yaml:
            if loaded_yaml['units']['length'] == 'cm' and loaded_yaml['units']['quantity'] == 'mol':
                ...
                if gas.reaction_type(i) in [4]:
                    ...
                    reactions[i]['A_0'] *= (1e-3) ** (reactant_stoich_coeffs[:, i].sum() - 1)
        ...
        kinetics_coeffs = KineticsCoeffs(arrhenius_coeffs=np.array(arrhenius_coeffs,dtype=np.float64),
        ...
        kinetics_data =  KineticsData(three_body_indices=np.array(three_body_indices,dtype=np.int64),
