...
def calculate_cp(T:Union[float, np.ndarray],
                 X: np.ndarray,
                 R:float,
    ...
    return CpData(cpdivR=cpdivR,
                 cp_mole=cp_mole)


def calculate_enthalpy(T:Union[float, np.ndarray],
                       X: np.ndarray,
                       R:float,
    ...
    """
    H_T = np.stack([1., T/2., (T**2)/3.,
                              (T**3)/4., (T**4)/5., 1. / T], axis=0)
...
def calculate_entropy(T:Union[float, np.ndarray],
                      P:float,
                      X: np.ndarray,
                      R:float,
    ...
    """
    S_T = np.stack([np.log(T),T, (T**2) / 2., (T**3)/ 3., (T**4) / 4., 1.], axis=0)
    ...
    s0 = sdivR*R
    s = partial_molar_entropies = s0 + R*(-np.log(X+1e-300)-np.log(P / 101325.))
    s_mole_array = s*X
